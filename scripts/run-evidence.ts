...
function parseArgs(argv: string[]) {
    ...
    if (a === "--n" && argv[i + 1]) {
      ...
      console.log(
        "Usage: tsx scripts/run-evidence.ts [--n N] [--out DIR] [--milestones m2,m3,m4,m5] [--write|--commit-artifacts] [--no-csv]\n" +
          "       tsx scripts/run-evidence.ts --sweep-m2 [--sweep-axes AXIS,...] [--n N] [--out DIR] [--write]\n" +
  ...
  return { n, outDir, milestones, csv, writeCommitted, sweepM2, sweepAxes };
}
...
function runSweep(args: ReturnType<typeof parseArgs>) {
  ...
  console.log(
    `Running M2 ladder-factor sweep expandAxes=${expandAxes.join(",")} n=${args.n ?? 1} → ${args.outDir}`,
  );
  const started = Date.now();
...
- Default expand: \`studyCondition\` only (cheap smoke). Full matrix: \`--sweep-axes ${M2_LADDER_SWEEP_AXES.join(",")}\`.
...
`;
  ...
  const elapsed = ((Date.now() - started) / 1000).toFixed(1);
  console.log(`Wrote ${results.length} M2 sweep arms under ${dir} in ${elapsed}s`);
}
