...
- **Write guard (P3 PASS tip @ `9f8c290`; initial @ `63fa427`):** default CLI writes `evidence/_smoke/` (gitignored). **Subpath refuse:** committed `evidence/{m2..m5}` (e.g. `evidence/m2`) require `--write`; only `_smoke` is writable without it.
