...
8. **Characterization→engine (locked)** — see section below; **C1 done @ `44cfaf9`**; **C2 done @ `2f45281`** (full θ + `schemaVersion` on all exports, fail-closed); **next C3** = M2 ladder-factor sweeps only
...
**Done:** C1 @ `44cfaf9` (ThetaV0 + golden replay); **C2 @ `2f45281`** (full ThetaV0 + `schemaVersion` on all evidence exports, CSV+JSONL fail-closed). **Next: C3** = M2 ladder-factor sweeps only (condition/controller/org/α/schedule). Observational ≠ closure; **M6 HARD-GATED**.
...
| 2026-09-25 | Editor | README + nav checkpoint after C2 landing (C2 done; C3 next = M2 ladder sweeps) | Done (this commit) |
