...
- **Engineering stamp (observational):** eng through **M5 organization scaffolding** is present; **evidence ladder M2–M5 shipped** with Reviewer **CONDITIONAL PASS** @ `0ac0d88` (`npm run evidence`, `evidence/m2`…`m5`). **Evidence-harden A–D tip @ `4d8f43c`:** A env-no-control + m2 arm; B calibrated observational K; C dual grid 48×36/72×54 + 20 explicit seeds; D `m5-coord-ablated`. Fixed `EVIDENCE_SEED_KEYS` (N=20). **Evidence-first work is in progress** — eng scaffolding ≠ scientific closure; Steps 3–5 / M2–M5 remain observational. **M6 HARD-GATED**. **Characterization→engine lock (captain ACCEPTED 2026-09-25):** C1–C3 before E1 eng; M2-only characterization first; θ v0 + env knobs in protocol; `schemaVersion` + full θ on evidence exports before sweeps; first sweep axes = ladder factors only (condition/controller/org/α/schedule), not continuous gains; no multi-system/meta-dynamics/UI theater; observational ≠ closure. **C1 landed @ `44cfaf9`** (ThetaV0 + golden replay); **C2 landed @ `2f45281`** (full ThetaV0 + `schemaVersion` on all exports, CSV+JSONL fail-closed); **C3 landed @ `51a88d2`** = M2 ladder-factor sweeps only (`m2-ladder-sweep`; continuous gains refused); **eng dual-grid expand @ `c1a979c`** (48×36+72×54 in one run; Reviewer PASS, nits OK); **eng schedule anatomy @ `f466e06`** (`sustainedShort`+`pulseLong` / `--sweep-schedule-levels`; Reviewer PASS nits); shared-disk smoke (gitignored; not committed): `m2-thick-20260925-212913/` (48-arm homeostatic thick) + `m2-thick-sc-20260925-222647/` (144-arm sc×thick; 144/144 ok) + `m2-sched-anat-20260925-225009/` (schedule-anatomy); **Characterization PARK** after schedule-anatomy (captain deferred; Coordinator park); verdict **duration-not-family** under homeostatic×Central×VB; banked CSVB homeostatic-only + duration not family/onset; **E1 HOLD**; **M6 HARD-GATED**; observational ≠ closure. **Write guard:** smoke defaults to `evidence/_smoke/` (gitignored); **subpath refuse** under `evidence/{m2..m5}` without `--write` (P3 PASS tip @ `9f8c290`; initial guard @ `63fa427`). Occupancy/CA UI language; Game of Life is not a product brand.
...
8. **Characterization→engine (locked)** — see section below; **C1–C3 done** (@ `44cfaf9` / `2f45281` / `51a88d2`); C3 = M2 ladder-factor sweeps only (continuous gains refused); **eng dual-grid expand @ `c1a979c`** (Reviewer PASS); **eng schedule anatomy @ `f466e06`** (Reviewer PASS nits); shared-disk smoke (gitignored; not committed): `m2-thick-20260925-212913/` + `m2-thick-sc-20260925-222647/` + `m2-sched-anat-20260925-225009/`; **Characterization PARK** after schedule-anatomy; verdict **duration-not-family**; **E1 HOLD**; **M6 HARD-GATED**; observational ≠ closure
...
**Done:** C1 @ `44cfaf9`; **C2 @ `2f45281`**; **C3 @ `51a88d2`** = M2 ladder-factor sweeps (`src/sim/m2-ladder-sweep.ts`, `npm run evidence -- --sweep-m2`). Axes: studyCondition / controllerMode / organizationMode / coordCouplingAlpha∈{0,0.3} / schedule∈{none,pulse,sustained}. Continuous gains refused. **Eng dual-grid expand @ `c1a979c`** (48×36+72×54 one run; Reviewer PASS, nits OK). **Eng schedule anatomy @ `f466e06`** (`sustainedShort`+`pulseLong` / `--sweep-schedule-levels`; Reviewer PASS nits). **Shared-disk smoke** (gitignored; not committed; no `--write`/promote): `evidence/_smoke/m2-thick-20260925-212913/` (48-arm homeostatic thick) + `evidence/_smoke/m2-thick-sc-20260925-222647/` (144-arm sc×thick; 144/144 ok) + `evidence/_smoke/m2-sched-anat-20260925-225009/` (schedule-anatomy). **Characterization PARK** after schedule-anatomy (captain deferred; Coordinator park). Verdict **duration-not-family** under homeostatic×Central×VB (not schedule family/onset). Banked: (1) CSVB homeostatic-only (2) duration not family/onset. **E1 HOLD**; **M6 HARD-GATED**; observational ≠ closure.
...
| 2026-09-25 | Editor | README + nav checkpoint after M2 thick expand | Done `552c501` |
| 2026-09-25 | Developer | Thick M2 shared-disk smoke land (48-arm N=20; org/α/schedule/controller; studyCondition fixed homeostatic; 48/48 ok) | Landed on shared disk `evidence/_smoke/m2-thick-20260925-212913/` (gitignored; not committed; not promoted; observational ≠ closure) |
| 2026-09-25 | Editor | Docs: distinguish eng tip `c1a979c` from thick M2 shared smoke land (overclaim fix) | Done `772de4e` |
| 2026-09-25 | Developer | sc×thick shared-disk land (studyCondition × thick axes; 144-arm; 144/144 ok) | Landed on shared disk `evidence/_smoke/m2-thick-sc-20260925-222647/` (gitignored; not committed; not promoted; observational ≠ closure) |
| 2026-09-25 | Captain/Coordinator | Characterization PARK after sc×thick (Expert lean park; dive caveat: prior thick homeostatic-only) | PARK |
| 2026-09-25 | Editor | Docs: characterization PARK stamp after sc×thick smoke | Done `4cf35d8` |
| 2026-09-25 | Developer | M2 schedule anatomy (`sustainedShort`+`pulseLong` / `--sweep-schedule-levels`) | Done `f466e06` |
| 2026-09-25 | Reviewer | PASS on schedule anatomy (nits OK) | Noted @ `f466e06` |
| 2026-09-25 | Developer | Schedule-anatomy shared-disk smoke land | Landed on shared disk `evidence/_smoke/m2-sched-anat-20260925-225009/` (gitignored; not committed; not promoted; observational ≠ closure) |
| 2026-09-25 | Captain/Coordinator | Characterization PARK after schedule-anatomy (captain deferred; Coordinator park); verdict duration-not-family under homeostatic×Central×VB; banked CSVB homeostatic-only + duration not family/onset | PARK |
| 2026-09-25 | Editor | Docs: characterization PARK stamp after schedule-anatomy | Done (this commit) |
