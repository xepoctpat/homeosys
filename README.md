...
- **C2 landed @ `2f45281`:** full ThetaV0 + `schemaVersion` on **all** evidence exports (CSV + JSONL fail-closed via `assertExportHasFullTheta`). **Next: C3** = M2 ladder-factor sweeps only (condition/controller/org/α/schedule). Still observational ≠ closure; **M6 HARD-GATED**. See [`docs/navigation-plan.md`](docs/navigation-plan.md).
...
| [`docs/theta-v0.md`](docs/theta-v0.md) | ThetaV0 characterization stamp (C1+C2) |
